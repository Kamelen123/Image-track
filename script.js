...
window.addEventListener("wheel", (e) => {
    ...
    const scrollSpeed = 0.5; // Adjust speed multiplier if needed
    const delta = e.deltaY * scrollSpeed; // Use deltaY to detect vertical scroll
    const maxDelta = window.innerWidth / 2;

    const percentage = (delta / maxDelta) * -100;
    const nextPercentage = Math.max(
        Math.min(parseFloat(track.dataset.percentage || "0") + percentage, 0),
        -100
    );

    track.dataset.percentage = nextPercentage;
    track.dataset.prevPercentage = nextPercentage; // Ensure next drag starts from latest position
    updateTrackPosition(nextPercentage);
});

// Function to update track position
function updateTrackPosition(percentage) {
    track.animate(
        {
...
images.forEach((image) => {
    image.addEventListener("click", () => {
        ...
        images.forEach((img) => {
            ...
            minimizedImage.addEventListener("click", () => {
                // Change background instead of recreating everything
...
document.querySelector('a[href="#work"]').addEventListener("click", (e) => {
    e.preventDefault(); // Prevent default anchor behavior
